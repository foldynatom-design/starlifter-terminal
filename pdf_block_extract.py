...
def draw_signatures(self):
    ...
    if current_y > (self.h - 85):
        self.add_page()
        current_y = self.get_y()
    # ── Minimum space needed for: story(~35) + telemetry(~35) + directive(5) + sigs(20) = ~95mm
    min_box_height = 95
    box_y = current_y + 4
    space_left = 276 - box_y
    if space_left < min_box_height:
        self.add_page()
        box_y = 20
        space_left = 256

    # Dynamic box height: use available space but cap so we don't waste page
    box_height = min(space_left, max(space_left, min_box_height))
    ...
    for item in items_list:
        ...
        # Skip large cargo crates (2+ SCU) — keep 1 SCU personal items
    ...
    # ── Telemetry panel on right column ──
    # Calculate bottom of telemetry = box_y + 45 (fixed height for right panel)
    tele_y = box_y + 11
    self.set_font("Roboto", "B", 7.5)
    self.set_text_color(140, 100, 30)
    self.text(142, tele_y, "HOLD TELEMETRY SENSORS:")
    self.set_font("Roboto", "", 6.5)
    self.set_text_color(60, 70, 90)
    tele_y += 5
    self.text(142, tele_y, "GRAVITY FIELD:")
    ...
    tele_y += 3
    self.text(142, tele_y, telemetry["gravity"])
    self.set_font("Roboto", "", 6.5)
    self.set_text_color(60, 70, 90)
    tele_y += 5
    self.text(142, tele_y, "ATM SEAL INTEGRITY:")
    self.set_font("Roboto", "B", 6.5)
    if "NOMINAL" in telemetry["atmosphere"]:
        ...
        self.set_text_color(231, 76, 60)
    tele_y += 3
    self.text(142, tele_y, telemetry["atmosphere"])
    self.set_font("Roboto", "", 6.5)
    self.set_text_color(60, 70, 90)
    tele_y += 5
    self.text(142, tele_y, "TRACTOR CLAMPS:")
    self.set_font("Roboto", "B", 6.5)
    if "LOCKED" in telemetry["clamps"]:
        ...
        self.set_text_color(231, 76, 60)
    tele_y += 3
    self.text(142, tele_y, telemetry["clamps"])
    self.set_font("Roboto", "", 6.5)
    self.set_text_color(60, 70, 90)
    tele_y += 5
    self.text(142, tele_y, "HAZMAT / RADIATION:")
    self.set_font("Roboto", "B", 6.5)
    if "CLEAR" in telemetry["hazmat"]:
        ...
        self.set_text_color(231, 76, 60)
    tele_y += 3
    self.text(142, tele_y, telemetry["hazmat"])
    
    # ── CARGO GRID PREVIEW (right column, below telemetry) ──
    ...
    # Grid preview box — positioned below telemetry
    grid_area_x = 142
    grid_area_y = tele_y + 5
    grid_area_w = 52
    grid_area_h = max(15, box_y + box_height - grid_area_y - 25)  # leave room for sig
    if grid_area_h > 5:
    ...
    # Use whichever is lower: paragraph_end_y or telemetry bottom
    content_bottom_y = max(paragraph_end_y + 2, tele_y + 5)
    # Ensure we leave at least 24mm for signatures at the bottom of the box
    sig_space_needed = 24  # sig block height
    directive_space = 10   # cargo directive + rec transport
    ...
    # ══════ PAGE 3: FULL-SIZE 3D ISOMETRIC CARGO GRID ══════
...
def generate_pdf_direct(self, save_path=None):
    ...
    # CVBG44 logo left
    bg44_logo = os.path.join(res_dir, "cvbg44_logo_dark.png")
    if not os.path.exists(bg44_logo):
        bg44_logo = os.path.join(res_dir, "cvbg44_logo.png")
    if os.path.exists(bg44_logo):
    ...
    # Barcode
    try:
        from pdf_engine import get_signatures_dir, get_processed_barcode_path
        _sr_pd = get_signatures_dir()
        _sr_bc = get_processed_barcode_path(_sr_pd)
        if _sr_bc and os.path.exists(_sr_bc):
            pdf.image(_sr_bc, x=145, y=29, w=45, h=8)
    except: pass
    ...
    except: pdf.set_font("Helvetica", "B", 5)
    pdf.set_text_color(100, 116, 139)
    pdf.text(10, 38, f"LEDGER HASH: {sr_hid}")
    
    # Classification badge
    sec_upper = classification.upper() if classification else ""
    ...
    if _sr_wm_file:
        _sr_wm_path = os.path.join(res_dir, _sr_wm_file)
    ...
    # ── METADATA ROWS (compact, same as manifest) ──
    ...
    if location: r1_parts.append(f"STATION: {location}")
    if r1_parts:
        pdf.text(meta_x, meta_y, "  |  ".join(r1_parts))
    
    # Row 2: OFFICER | CAPTAIN
    r2_parts = []
    ...
    for item in items:
        ...
        # 5) General equipment fallback: weapons, tools, food, armor
        if not best_loc:
            _equip_map = {
                # Weapons & ammo — Centermass is primary weapons dealer
    ...
    for item in items:
        ...
        elif "cscu" in box_sz: total_scu += qty * 1  # legacy cscu = 1 SCU grid slot min
    ...
    try:
        if is_eva:
            ...
            if cargo_rec and cargo_rec.get("note"):
                try: pdf.set_font("Roboto", "I", 6)
                except: pdf.set_font("Helvetica", "I", 6)
                pdf.set_text_color(120, 100, 50)
                pdf.text(12, row_y + 10, cargo_rec["note"][:130])
                row_y += 6
    except Exception:
        pass
